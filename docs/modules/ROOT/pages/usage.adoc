...
== Setting the target factory_bot version

Some cops only apply to code that targets a recent enough factory_bot, and are
skipped otherwise. `FactoryBot/RedundantEnumTrait`, for example, needs
factory_bot 6.0.

The version is taken from `Gemfile.lock` or `gems.locked`. When neither is
available, or the lockfile does not mention factory_bot, RuboCop assumes
factory_bot 6.0. Set it explicitly to override both:

[source,yaml]
----
AllCops:
  TargetFactoryBotVersion: 5.2
----
